...
# Rank requirements as specified in your request
RANKS = {
    1: {"name": "Rank 1", "xp": 50},
    ...
    10: {"name": "The Legend", "xp": float('inf')}, # The final rank
}
...
def get_rank_info(xp):
    ...
    if current_rank_num == 0:
        xp_for_current_rank = 0
    else:
        xp_for_current_rank = RANKS[current_rank_num-1]['xp'] if current_rank_num > 1 else 0

    current_rank_name = RANKS.get(current_rank_num, {}).get('name', "Unranked")
    
    return current_rank_name, xp_for_current_rank, xp_for_next_rank
...
class RankingCog(commands.Cog, name="Ranking"):
    ...
    async def voice_xp_loop(self):
        ...
        for guild in self.bot.guilds:
            for channel in guild.voice_channels:
                active_members = [m for m in channel.members if not m.bot and not m.voice.self_deaf and not m.voice.self_mute]
    ...
    async def leaderboard(self, interaction: discord.Interaction):
        ...
        for i, (user_id, xp) in enumerate(top_users):
            ...
            if member:
                description += f"**{i+1}.** {member.mention} - `{xp}` XP ({rank_name})\n"
            else:
                description += f"**{i+1}.** *Unknown User ({user_id})* - `{xp}` XP ({rank_name})\n"
