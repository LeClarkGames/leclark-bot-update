...
# --- NEW: View for claiming ownership ---
class ClaimOwnershipView(discord.ui.View):
    ...
    async def claim_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # Give the new owner channel management permissions and remove them from the old owner
        overwrites = self.channel.overwrites
        old_owner_id = next((uid for uid, perms in overwrites.items() if perms.manage_channels), None)
        if old_owner_id:
...
class TempVCCog(commands.Cog, name="Temp VCs"):
    ...
    async def on_voice_state_update(self, member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
        ...
        if after.channel and after.channel.id == hub_channel_id:
            ...
            try:
                ...
                # --- UPDATED: More detailed user guide ---
                ...
                guide_embed.add_field(name="`/vc lock`", value="Lock the channel so no one new can join.", inline=False)
                guide_embed.add_field(name="`/vc unlock`", value="Unlock the channel for anyone to join.", inline=False)
                guide_embed.add_field(name="`/vc permit <user/role>`", value="Allow a specific user or role to join, even if it's locked.", inline=False)
                guide_embed.add_field(name="`/vc deny <user/role>`", value="Block a user/role from joining. This also kicks them.", inline=False)
                guide_embed.add_field(name="`/vc rename <name>`", value="Change the name of the channel.", inline=False)
                guide_embed.add_field(name="`/vc limit <number>`", value="Set a user limit (0 for unlimited).", inline=False)
        ...
        # --- UPDATED: Channel Deletion & Ownership Transfer Logic ---
        if before.channel:
            ...
            if member.id == owner_id and len(before.channel.members) > 0:
                ...
                message = await before.channel.send("The channel owner has left. Someone else can claim ownership.", view=view)
    ...
    # --- NEW: Listener for inappropriate channel names ---
    @commands.Cog.listener()
    async def on_guild_channel_update(self, before: discord.abc.GuildChannel, after: discord.abc.GuildChannel):
        if not isinstance(after, discord.VoiceChannel) or before.name == after.name:
            return

        owner_id = await database.get_temp_vc_owner(after.id)
        if not owner_id:
            return

        bad_words = await database.get_bad_words(after.guild.id)
        if any(word in after.name.lower() for word in bad_words):
            owner = after.guild.get_member(owner_id)
            if owner:
                default_name = f"┃ {owner.display_name}'s Room"
                await after.edit(name=default_name, reason="Inappropriate name detected.")
                
                if owner.voice and owner.voice.channel == after:
                    await owner.move_to(None, reason="Kicked for setting inappropriate VC name.")
                
                try:
                    await owner.send(f"Your temporary voice channel in **{after.guild.name}** was renamed and you were kicked because the name contained a forbidden word. The channel name has been reset.")
                except discord.Forbidden:
                    pass
