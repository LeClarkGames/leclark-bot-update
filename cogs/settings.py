...
class BaseSettingsView(discord.ui.View):
    ...
    async def refresh_and_show(self, interaction: discord.Interaction, edit_original: bool = False):
        ...
        if edit_original and hasattr(self, 'message') and self.message is not None:
             message_to_edit = self.message

        if interaction.response.is_done():
            await message_to_edit.edit(embed=embed, view=target_view)
        else:
...
class SettingsMainView(BaseSettingsView):
    ...
    # --- (All existing buttons for Channels, Roles, etc. are here) ---
    ...
    async def verification_settings(self, interaction: discord.Interaction, button: discord.ui.Button): await interaction.response.edit_message(content="Configure the member verification system.", view=VerificationSettingsView(self.bot, self))
...
class ShopSettingsView(BaseSettingsView):
    def __init__(self, bot: commands.Bot, parent_view: SettingsMainView):
        super().__init__(bot, parent_view)
        self.message: Optional[discord.Message] = None
        self.add_item(RoleSelect("custom_role_divider_role_id", "Set Custom Role Divider", self))

    async def get_shop_embed(self, guild: discord.Guild):
        settings = await database.get_all_settings(guild.id)
        cost = settings.get('custom_role_cost', 100)
        divider_id = settings.get('custom_role_divider_role_id')
        divider_role = f"<@&{divider_id}>" if divider_id else "Not Set"

        embed = discord.Embed(title="⚔️ Shop Settings", color=config.BOT_CONFIG["EMBED_COLORS"]["INFO"])
        desc = "Configure the KOTH Points Shop.\n\n"
        desc += f"**Custom Role Cost:** `{cost}` points.\n"
        desc += f"**Divider Role:** {divider_role}\n*Custom roles will be created just below this role in the hierarchy.*"
        
        embed.description = desc
        return embed

    @discord.ui.button(label="Set Role Cost", style=discord.ButtonStyle.secondary)
    async def set_role_cost(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(ShopCostModal(self))

class ShopCostModal(discord.ui.Modal, title="Set Custom Role Cost"):
    def __init__(self, parent_view: ShopSettingsView):
        super().__init__()
        self.parent_view = parent_view
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await database.update_setting(interaction.guild.id, 'custom_role_cost', cost)
        await interaction.response.send_message(f"✅ Custom role cost set to **{cost}** points.", ephemeral=True)
        await self.parent_view.refresh_and_show(interaction, edit_original=True)
...
class SettingsCog(commands.Cog):
    ...
    # Inside the SettingsCog class in settings.py

    @app_commands.command(name="widget_token", description="Gets the secret token for the web widget.")
    @is_bot_admin()
    async def widget_token(self, interaction: discord.Interaction):
        """Provides the secret token needed for the web widget URL."""
        await interaction.response.defer(ephemeral=True)
        
        token = await database.get_or_create_widget_token(interaction.guild.id)
        
        embed = discord.Embed(
            title="🔑 Web Widget Token",
            description="This is your server's secret token. **Do not share it publicly!**",
        ...
        embed.add_field(
            name="Example URL",
            value=f"`http://YOUR_IP:PORT/widget/{interaction.guild.id}?token={token}`",
            inline=False
        )
