...
async def get_panel_embed_and_view(guild: discord.Guild, bot: commands.Bot):
    """
    Helper function to generate the correct panel embed and view based on the database state.
    """
    ...
    if status.startswith('koth'):
        ...
        if status == 'koth_tiebreaker':
            ...
            user1_id, user2_id = list(cog.tiebreaker_users.get(guild.id, (None, None)))
            user1 = guild.get_member(user1_id)
            user2 = guild.get_member(user2_id)
            if user1 and user2:
                desc += f"\n\nWaiting for final submissions from {user1.mention} and {user2.mention}."

        desc += f"\n**Queue:** `{koth_queue_count}` challengers pending."
        
        if guild.id in cog.current_king:
            king_user = guild.get_member(cog.current_king[guild.id]['user_id'])
            desc += f"\n\n**Current King:** {king_user.mention if king_user else 'Unknown'}"
        ...
        if status == 'koth_open':
            ...
            if session_stats:
                ...
                for i, (user_id, stats) in enumerate(sorted_session[:5]):
                    ...
                    desc += f"`{i+1}.` {user.display_name if user else 'Unknown User'}: `{stats['points']}` points (`{stats['wins']}` wins)\n"
            else:
                desc += "No points scored in this battle yet.\n"

        embed = discord.Embed(title="⚔️ King of the Hill Panel", description=desc, color=config.BOT_CONFIG["EMBED_COLORS"]["INFO"])
    ...
    # Regular Mode Embed
        ...
        embed = discord.Embed(
            ...
            description=f"Submissions are currently **{'OPEN' if is_open else 'CLOSED'}**.\n\n**Queue:** `{queue_count}` tracks pending.",
            color=config.BOT_CONFIG["EMBED_COLORS"]["SUCCESS"] if is_open else config.BOT_CONFIG["EMBED_COLORS"]["ERROR"]
        )
...
class KOTHBattleView(discord.ui.View):
    ...
    async def _handle_vote(self, interaction: discord.Interaction, winner: str):
        ...
        winner_data = self.king if winner == 'king' else self.challenger
        loser_data = self.challenger if winner == 'king' else self.king
        winner_user = interaction.guild.get_member(winner_data['user_id'])
        ...
        if winner_id not in session_stats:
            session_stats[winner_id] = {'points': 0, 'wins': 0}
        session_stats[winner_id]['points'] += 1
        session_stats[winner_id]['wins'] += 1

        await database.update_submission_status(self.king['submission_id'], 'reviewed', interaction.user.id)
        await database.update_submission_status(self.challenger['submission_id'], 'reviewed', interaction.user.id)
        self.cog.current_king[interaction.guild.id] = winner_data

        await interaction.message.delete()
        self.cog.battle_in_progress.discard(interaction.guild.id)
        ...
        winner_message = await interaction.followup.send(f"👑 **{winner_user.display_name if winner_user else 'Someone'}** wins the round and is the new King of the Hill!")
        self.cog.koth_battle_messages[interaction.guild.id].append(winner_message.id)

    @discord.ui.button(label="👑 King/Track 1 Wins", style=discord.ButtonStyle.primary)
    async def king_wins(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self._handle_vote(interaction, 'king')

    @discord.ui.button(label="⚔️ Challenger/Track 2 Wins", style=discord.ButtonStyle.success)
    ...
    async def skip(self, interaction: discord.Interaction, button: discord.ui.Button):
        if self.is_tiebreaker:
             return await interaction.response.send_message("❌ You cannot skip a tiebreaker match.", ephemeral=True)
        if not await utils.has_mod_role(interaction.user):
            return await interaction.response.send_message("❌ You do not have permission to skip.", ephemeral=True)
        
        await database.update_submission_status(self.challenger['submission_id'], 'skipped', interaction.user.id)
        await interaction.message.delete()
        self.cog.battle_in_progress.discard(interaction.guild.id)
        await interaction.response.send_message("✅ Challenger skipped.", ephemeral=True)
...
class SubmissionPanelView(discord.ui.View):
    ...
    async def play_koth_queue(self, interaction: discord.Interaction):
        ...
        if guild_id in self.cog.battle_in_progress:
            return await interaction.response.send_message("❌ A battle is already in progress. Please resolve it first.", ephemeral=True)
        
        if guild_id not in self.cog.current_king:
            king_track = await database.get_next_submission(guild_id, 'koth')
            if not king_track: return await interaction.response.send_message("The KOTH queue is empty! Need at least one challenger.", ephemeral=True)
            
            sub_id, user_id, url = king_track
            self.cog.current_king[guild_id] = {"user_id": user_id, "submission_id": sub_id, "track_url": url}
            await database.update_submission_status(sub_id, 'reviewing', interaction.user.id)
            king_user = interaction.guild.get_member(user_id)
            embed = discord.Embed(title="👑 New King of the Hill!", description=f"The first challenger has entered! **{king_user.display_name}** is the new King of the Hill!", color=config.BOT_CONFIG["EMBED_COLORS"]["SUCCESS"])
            
            await interaction.response.send_message(content=url, embed=embed)
            message = await interaction.original_response()
            self.cog.koth_battle_messages[guild_id].append(message.id)
            await self._update_panel(interaction)
        else:
            ...
            king_data = self.cog.current_king[guild_id]
            challenger_data = {"user_id": c_user_id, "submission_id": c_sub_id, "track_url": c_url}
            
            king_user = interaction.guild.get_member(king_data['user_id'])
            challenger_user = interaction.guild.get_member(c_user_id)
            
            embed = discord.Embed(title="⚔️ BATTLE TIME! ⚔️", color=discord.Color.gold())
            embed.add_field(name=f"👑 The King: {king_user.display_name if king_user else 'Unknown'}", value=f"Track: {king_data['track_url']}", inline=False)
            ...
            self.cog.battle_in_progress.add(guild_id)
    ...
    async def stop_koth_battle(self, interaction: discord.Interaction):
        ...
        is_tie = False
        if len(sorted_session) > 1 and sorted_session[0][1]['points'] > 0:
            if sorted_session[0][1]['points'] == sorted_session[1][1]['points']:
                is_tie = True

        if is_tie:
            user1_id, user2_id = sorted_session[0][0], sorted_session[1][0]
            self.cog.tiebreaker_users[guild_id] = {user1_id, user2_id}
            self.cog.tiebreaker_submissions[guild_id] = {}
            ...
            if (koth_channel_id := await database.get_setting(guild_id, 'koth_submission_channel_id')) and (channel := self.bot.get_channel(koth_channel_id)):
                await channel.send(f"**⚔️ TIEBREAKER! ⚔️**\nWe have a tie for first place! {user1.mention} and {user2.mention}, you must each submit one final track to determine the winner.")
            
            await interaction.followup.send("A tiebreaker has been initiated!", ephemeral=True)
            return

        winner_id = sorted_session[0][0] if sorted_session else None
...
class SubmissionsCog(commands.Cog, name="Submissions"):
    def __init__(self, bot: commands.Bot):
        ...
        self.battle_in_progress = set()
        self.tiebreaker_users = {}
        self.tiebreaker_submissions = {}
    ...
    async def finalize_koth_battle(self, interaction: discord.Interaction, winner_id: int | None):
        ...
        if review_channel_id := await database.get_setting(guild_id, 'review_channel_id'):
            if review_channel := self.bot.get_channel(review_channel_id):
                message_ids_to_delete = self.cog.koth_battle_messages.pop(guild_id, [])
        ...
        self.current_king.pop(guild_id, None)
        self.current_koth_session.pop(guild_id, None)
        self.tiebreaker_users.pop(guild_id, None)
    ...
    async def on_ready(self):
        log.info("Submissions cog is ready. Checking for active KOTH sessions...")
        if not self.bot.is_ready():
            await self.bot.wait_until_ready()

        for guild in self.bot.guilds:
            status = await database.get_setting(guild.id, 'submission_status')
            if status == 'koth_open':
                # Logic to restore KOTH king state on restart
                pass
    ...
    async def on_message(self, message: discord.Message):
        ...
        if status == 'open' and message.channel.id == submission_channel_id:
            ...
            if message.author.id in self.tiebreaker_users.get(message.guild.id, set()):
                ...
                if attachment.content_type and attachment.content_type.startswith("audio/"):
                    ...
                    if submission_type == 'koth':
                        ...
                        if user_id not in session_stats:
                            session_stats[user_id] = {'points': 0, 'wins': 0, 'submissions': 0}
                        session_stats[user_id]['submissions'] += 1
